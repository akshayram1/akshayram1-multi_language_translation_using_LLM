...
pip install poetry 
```
In case poetry command is not working in next step (due to permission issues at local system) then create virtual env and follow following steps 

```bash
python -m venv .venv
venv\Scripts\activate (Widnow)
...
# In case above command throws 'NO translation_agent module' then follow below steps
python -m pip install streamlit
python -m streamlit run app.py
```

For Python related execution

```python
import translation_agent as ta
source_lang, target_lang, country, model = "English", "Spanish", "Mexico", "gemini-pro"
translation = ta.translate(source_lang, target_lang, source_text, country, model)
```
...
# akshayram1-multi_language_translation_using_LLM
